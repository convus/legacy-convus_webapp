  ...
  context "logged in" do
        ...
        context "with hypothesis params" do
          ...
          context "with hypothesis relation params" do
            ...
            it "updates hypothesis" do
              ...
              expect(hypothesis_relation.hypothesis_earlier_id).to eq hypothesis.id
    ...
    describe "update" do
      ...
      context "update hypothesis" do
        ...
        context "with hypothesis relations" do
          it "updates" do
            HypothesisRelation.find_or_create_for(kind: "hypothesis_support", hypotheses: [hypothesis, hypothesis_related])
            subject.reload
            expect(subject.editable_by?(current_user)).to be_truthy
            expect(subject.text).to_not eq explanation_with_quote_params[:text]
            expect(subject.explanation_quotes.count).to eq 0
            ...
            expect(hypothesis.reload.title).to eq "This seems like the truth."
            expect(hypothesis.tags.pluck(:title)).to eq(["Economy", "parties"])
            # Make sure the initial relation doesn't stick around
            expect(hypothesis.relations.count).to eq 1
            hypothesis_relation = hypothesis.relations.last
            expect(hypothesis_relation.hypothesis_later_id).to eq hypothesis_related.id
