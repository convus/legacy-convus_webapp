...
RSpec.describe GitContentRepo do
  let(:subject) { described_class }

  describe "output_success?" do
    it "returns true" do
      expect(described_class.output_success?("\n\n\nEverything up-to-date \n")).to be_truthy
      expect(described_class.output_success?("\n\n \n")).to be_truthy
    end
    context "all up to date" do
      it "returns true" do
        expect(described_class.output_success?("already up to date ")).to be_truthy
        expect(described_class.output_success?("Already up to date.")).to be_truthy
      end
    end
    context "multiline" do
      ...
      it "returns truthy" do
        expect(described_class.output_success?(multiline)).to be_truthy
      end
    end
    ...
    context "and another" do
      ...
      it "returns truthy" do
        expect(described_class.output_failed?(multiline)).to be_falsey
      end
    end
